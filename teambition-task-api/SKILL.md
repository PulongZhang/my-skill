...
当前 `work-time-fill-template.js` 是兜底实现：每个日期仅支持一个任务。一天需要在多个任务间分配工时时，应先生成显式分配方案和针对该方案的脚本，不要直接套用模板。
...
2. 对复用任务逐项校验 ID、标题、项目、SFC、执行者、删除/归档/完成状态；SFC 必须纳入授权方案，变化时停止。归档状态 `tasks/bulk` 不返回，必须用 GraphQL 的 `isArchived` 校验。发工时前再对所有最终任务 ID 做一次 bulk 预检，异常项不得写入。
