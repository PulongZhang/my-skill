...
用法：
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_server_transfer.py <源别名> <源路径> <目标别名> <目标路径> [options]

示例：
    # 自动模式（推荐）
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_server_transfer.py prod-web-01 /var/log/app.log backup-server /backup/

    # 强制直连模式（大文件推荐）
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_server_transfer.py prod-web-01 /data/large.tar.gz backup-server /backup/ --mode direct

    # 强制流式转发（小文件或网络不通时）
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_server_transfer.py prod-web-01 /etc/config.conf backup-server /backup/ --mode stream

    # 混合模式（先尝试直连，失败后自动降级到流式）
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_server_transfer.py prod-web-01 /data/ backup-server /backup/ --mode hybrid

    # 使用 rsync（仅直连模式支持）
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_server_transfer.py prod-web-01 /data/ backup-server /backup/data/ --use-rsync
"""
...
from security import (configure_host_key_verification, quote_posix_shell_arg,
                      validate_network_host, validate_port)
...
def get_remote_file_size(alias, remote_path):
    ...
    # 先尝试文件，再尝试目录
    path_arg = quote_posix_shell_arg(remote_path)
    result = client.execute(
        f"stat -c %s {path_arg} 2>/dev/null || "
        f"du -sb {path_arg} 2>/dev/null | awk '{{print $1}}'"
    )
...
def is_remote_directory(alias, remote_path):
    ...
    client = create_ssh_client(alias)
    result = client.execute(
        f"test -d {quote_posix_shell_arg(remote_path)} && echo 'DIR' || echo 'FILE'"
    )
...
def can_servers_connect(source_alias, dest_alias):
    ...
    try:
        ...
        dest_host = validate_network_host(dest_params['hostname'])
        dest_port = validate_port(dest_params['port'])

        client = create_ssh_client(source_alias)
        tcp_check = quote_posix_shell_arg(
            f"cat < /dev/null > /dev/tcp/{dest_host}/{dest_port}"
        )
        test_cmd = f"timeout 5 bash -c {tcp_check} 2>/dev/null && echo 'OK' || echo 'FAIL'"
...
def direct_transfer(source_alias, source_path, dest_alias, dest_path,
    ...
    # Paramiko exec_command 仍通过远程 shell 执行，所有动态值必须独立引用。
    dest_port = validate_port(dest_port)
    source_arg = quote_posix_shell_arg(source_path)
    destination_arg = quote_posix_shell_arg(f"{dest_user}@{dest_host}:{dest_path}")
    if use_rsync:
        transport_arg = quote_posix_shell_arg(
            f"ssh -p {dest_port} -o StrictHostKeyChecking=yes"
        )
        cmd = f"rsync -avz --progress -e {transport_arg} {source_arg} {destination_arg}"
    else:
        cmd = (
            f"scp -o StrictHostKeyChecking=yes -P {dest_port} "
