...
用法：
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_tunnel.py start <alias> --remote-port <port> [--local-port <port>] [--remote-host <host>]
    ...
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_tunnel.py stop-all <alias>
...
示例：
    ...
    # 指定本地端口
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_tunnel.py start prod-db-01 --local-port 3306 --remote-port 3306

    # 转发到远程的其他主机
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_tunnel.py start prod-web-01 --remote-host 192.168.1.100 --remote-port 8080

    # 查看所有活动的 tunnel
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_tunnel.py list

    # 停止特定 tunnel
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_tunnel.py stop prod-db-01-3306

    # 停止服务器的所有 tunnel
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_tunnel.py stop-all prod-db-01
"""
...
from security import configure_host_key_verification
...
class SSHTunnel:
    ...
    def _connect_ssh(self):
        ...
        configure_host_key_verification(self._ssh_client)
    ...
    def _create_proxy_client(self, proxy_jump: str):
        ...
        configure_host_key_verification(proxy_client)
