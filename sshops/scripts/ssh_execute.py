...
用法：
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_execute.py <alias> <command> --confirm [--timeout TIMEOUT]
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_execute.py <alias> <command> --no-daemon --confirm

示例：
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_execute.py prod-web-01 "whoami && hostname" --confirm
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_execute.py DEV-002 "df -h" --timeout 60 --confirm
"""
...
from security import audit_command, is_dangerous_command
...
def try_daemon_execute(alias, command, timeout, confirmed=False):
...
def main():
    ...
    parser.add_argument('--confirm', action='store_true',
                        help='Acknowledge a destructive or privileged remote command')

    args = parser.parse_args()
    timeout = args.timeout or 30

    # Arbitrary remote shell text cannot be safely classified with a blacklist.
    # Require an explicit acknowledgement for every execution request.
    if not args.confirm:
        audit_command(args.alias, args.command, execution='rejected', confirmed=False,
                      outcome='confirmation_required')
        parser.error('远程命令会由目标 shell 解释；确认影响后请显式添加 --confirm')
    ...
    try:
        ...
        if use_daemon:
            # 密码认证：尝试通过守护进程执行
            result = try_daemon_execute(args.alias, args.command, timeout, args.confirm)
        ...
        audit_command(args.alias, args.command,
                      execution='daemon' if use_daemon else 'direct',
                      confirmed=args.confirm,
                      outcome={'success': result.get('success'), 'exit_code': result.get('exit_code')})
