...
用法：
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_cluster.py <command> --confirm [--parallel] [--hosts HOSTS] [--environment ENV]

示例：
    # 对所有服务器执行命令
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_cluster.py "uptime" --parallel --confirm

    # 对指定别名列表执行
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_cluster.py "df -h" --hosts "DEV-002,DEV-003" --parallel --confirm

    # 按环境过滤
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_cluster.py "uptime" --environment production --parallel --confirm

    # 健康检查
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_cluster.py "systemctl status nginx" --parallel --confirm --health-check
"""
...
def main():
    ...
    if not args.confirm:
        audit_command('*', args.command, execution='cluster-rejected', confirmed=False,
                      outcome='confirmation_required')
        parser.error('远程命令会由目标 shell 解释；确认受影响主机后请显式添加 --confirm')
    ...
    try:
        ...
        if args.health_check:
            ...
            for alias, result in results.items():
                audit_command(alias, args.command, execution='cluster',
