...
用法：
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_daemon.py start <alias>
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_daemon.py status <alias>
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_daemon.py stop <alias>

示例：
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_daemon.py start DEV-002
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_daemon.py status DEV-002
    uv run --project ~/.claude/skills/sshops python ~/.claude/skills/sshops/scripts/ssh_daemon.py stop DEV-002
"""
...
from security import audit_command, configure_host_key_verification, is_dangerous_command
    ...
    def _handle_client(self, client_sock: socket.socket):
        ...
        try:
            ...
            if not hmac.compare_digest(str(request.get('token', '')), self._token):
                _send_message(client_sock, {'success': False, 'stderr': 'unauthorized'})
                return
            ...
            if action == 'ping':
                ...
                if not confirmed:
                    result = {
                        ...
                        'stderr': '远程命令会由目标 shell 解释；需要 --confirm'
                    }
                    audit_command(self.alias, command, execution='daemon',
                                  confirmed=False, outcome='confirmation_required')
                else:
                    self._last_activity = time.time()
                    ...
                    audit_command(self.alias, command, execution='daemon',
                                  confirmed=confirmed,
                                  outcome={'success': result['success'], 'exit_code': result['exit_code']})
...
def cmd_stop(alias: str):
    ...
    try:
        ...
        _send_message(sock, {'action': 'shutdown', 'token': info.get('token', '')})
...
def cmd_status(alias: str):
    ...
    try:
        ...
        sock.settimeout(5)
        sock.connect(('127.0.0.1', info['port']))
        _send_message(sock, {'action': 'ping', 'token': info.get('token', '')})
        resp = _recv_message(sock, timeout=5)
        sock.close()
