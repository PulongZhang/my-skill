...
```bash
python ~/.claude/skills/ssh-skill/scripts/ssh_interactive.py <别名> --confirm
```

可在连接后自动执行一个初始命令：

```bash
python ~/.claude/skills/ssh-skill/scripts/ssh_interactive.py <别名> \
...
交互式会话不会使用 daemon，也不会记录终端输入、屏幕输出或密码；只审计会话元数据和初始命令摘要。对于非交互的 Arthas 执行，必须设置 `watch -n <采样次数>` 与 `ssh_execute.py --timeout <秒>`，避免无限挂起。
