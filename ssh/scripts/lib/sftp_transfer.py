...
class SFTPTransfer:
    ...
    def download_directory(self, remote_dir: str, local_dir: str,
        ...
        local_dir = os.path.realpath(os.path.abspath(local_dir))
        os.makedirs(local_dir, exist_ok=True)
        self._download_root = local_dir
    ...
    def _download_dir_recursive(self, remote_dir: str, local_dir: str,
        ...
        for entry in entries:
            # SFTP server responses are untrusted; one entry must be one path component.
            name = entry.filename
            if (not name or name in ('.', '..') or '\x00' in name or '/' in name or '\\' in name):
                result.files_failed += 1
                result.errors.append(f"拒绝不安全的远程文件名: {name!r}")
                continue
            remote_path = remote_dir.rstrip('/') + '/' + name
            local_path = os.path.realpath(os.path.join(local_dir, name))
            try:
                if os.path.commonpath([self._download_root, local_path]) != self._download_root:
                    raise ValueError("下载路径越过目标目录")
            except ValueError:
                result.files_failed += 1
                result.errors.append(f"拒绝越界下载路径: {name!r}")
                continue
