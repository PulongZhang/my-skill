...
class ParamikoClient:
    ...
    def __init__(
        ...
        # 密码仅在 Paramiko 进程内使用；不得落盘为 SSH_ASKPASS 脚本。
    ...
    def _create_password_script(self) -> str:
        """Deprecated: password-backed SSH_ASKPASS scripts are intentionally disabled."""
        raise RuntimeError('不支持将 SSH 密码写入 SSH_ASKPASS 临时脚本')
    ...
    def _get_env_with_password(self) -> Dict[str, str]:
        """Return a copy of the environment without injecting password material."""
    ...
    def _connect_through_jump_hosts(self) -> paramiko.SSHClient:
        ...
        try:
            ...
            configure_host_key_verification(current_client)
            ...
            for jump in self.jump_hosts[1:]:
                ...
                configure_host_key_verification(next_client)
