...
# Security validation for values persisted to ~/.ssh/config.
...
try:
    ...
    print("请在 Skill 根目录执行: uv sync", file=sys.stderr)
    sys.exit(1)
...
class SSHConfigManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # 密码元数据可位于 config 注释中，因此配置和目录必须仅当前用户可读。
    ...
    def create_host(self, alias: str, hostname: str, user: str,
        ...
        Returns:
            是否成功创建
        """
        # SSH config 是可执行配置语言；拒绝换行等指令注入字符。
        alias = validate_ssh_config_value(alias, 'alias', single_token=True)
        hostname = validate_ssh_config_value(hostname, 'hostname', single_token=True)
        user = validate_ssh_config_value(user, 'user', single_token=True)
        identity_file = validate_ssh_config_value(identity_file, 'identity_file')
        proxy_jump = validate_ssh_config_value(proxy_jump, 'proxy_jump', single_token=True)
    ...
    def update_host(self, alias: str,
        ...
        location = validate_ssh_config_value(location, 'location')
        if tags is not None:
            tags = [validate_ssh_config_value(tag, 'tag', single_token=True) for tag in tags]
        if port is not None:
            port = validate_port(port)
