...
3. **多代理并行评审（自动）**：派发多个评审子代理（Agent tool，并行启动），每个代理只负责一个维度（维度表见下），互不重叠，各自返回带**证明**的 findings。子代理只读：只读 diff 和必要上下文，不改文件、不跑测试。
4. **合并与复核**：汇总各代理 findings，去重、按严重程度分级。每条 `【必改】` 必须附证明（触发场景 + 代码证据链 + 预期对比）；发布前复核证明成立——引用的 `file:line` 真实存在、调用链/数据流与代码一致、预期差异属实；复核不过的降级或删除。**精确率优先于召回率**：可用性要求宁可不报、不可错报——对证明存疑的一律删除，不靠多报提高覆盖率（漏报代价是作者稍后自己发现，错报代价是误导排查 + 评审信誉受损）。
...
- **证据链**：`file:line` 引用 + 关键调用路径（数据从哪来、流向哪、在哪出错）。从 diff 看不出上下文时必须 `git show` / 读文件核实后再下结论，不凭 diff 片段猜测。
