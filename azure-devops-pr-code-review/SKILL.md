...
5. 用户明确说“发评论/作出评论/评论到 PR”时，再发布：1 条总结评论 + 每个 `【必改】` 具体问题 1 条行内评论（必须钉到具体 `file:line`）；`【建议】`/`【确认】`/`【提示】` 只写入总结评论，不发布行内评论。
...
- **每次发布到 PR 必须先发 1 条总结评论**：总结评论覆盖全 PR，并列出 `【必改】`/`【建议】`/`【确认】`/`【提示】`。
- **行内评论只用于 `【必改】`**：每个必改具体问题 1 条行内评论（必须钉到具体 `file:line`）；`【建议】`/`【确认】`/`【提示】` 不写行内评论，只写在总结评论里。
