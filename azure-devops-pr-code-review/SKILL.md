...
每个评审代理只审查一个维度，避免单视角漏检。按 PR 规模裁剪：小 PR 至少派 **正确性、安全** 两个；大 PR（改动文件多 / 跨模块 / 涉数据）派全部；PR 明显不涉及的维度不派发（如纯文案改动不派并发、性能）。

| 维度 | 该代理只关注 | 典型检查点 |
|---|---|---|
| 正确性 | 逻辑与边界 | 分支条件、空值/默认值、状态流转、数值/集合运算、边界下标 |
| 安全 | 安全风险 | 注入、鉴权/越权、敏感信息泄露、路径穿越、硬编码凭据、反序列化 |
| 并发/事务 | 竞态与一致性 | 共享状态、锁、事务边界、回滚、幂等、批处理 |
| i18n | 国际化 | 硬编码文案、字符编码、时间/数字/时区格式化 |
| 性能 | 资源与耗时 | N+1、循环内查库/调用、大对象拷贝、缺失索引 |
| 兼容性/回归 | 破坏性变更 | 依赖/API 升级、配置变更、历史数据、迁移脚本、回滚风险 |
