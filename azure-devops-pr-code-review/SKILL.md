...
3. **多代理并行评审（自动）**：派发多个评审子代理（Agent tool，并行启动），每个代理只负责一个维度（维度表见下），互不重叠，各自返回带**证明**的 findings。子代理只读：只读 diff 和必要上下文，不改文件、不跑测试；可用仓库级工具核实证据（`git grep` / `git show` / `git log` / `git blame`、REST 取文件）。
4. **合并与对抗性复核**：汇总各代理 findings，去重、按严重程度分级。每条候选 `【必改】` 必须附证明（触发场景 + 仓库级证据链 + 预期对比）；发布前给每条派发一个独立**反驳代理**（与评审代理分离，只读）尽量证伪——核对引用的 `file:line` 是否真实存在、调用链/数据流是否与代码一致、预期差异是否属实、有无反例或作者可辩护的理由；反驳不动才保留，被反驳即删除或降级。**验证比生成便宜**（反驳只需针对性检查），所以每条必改都值得过这一关，这是精确率的最后防线。**精确率优先于召回率**：可用性要求宁可不报、不可错报——对证明存疑的一律删除，不靠多报提高覆盖率（漏报代价是作者稍后自己发现，错报代价是误导排查 + 评审信誉受损）。
5. 先形成中文评审结论；用户只说”评审/看看”时，先把结论发给用户，不直接发布到 PR。
6. 用户明确说”发评论/作出评论/评论到 PR”时，再发布：**只发布每个 `【必改】` 具体问题的行内评论**（必须钉到具体 `file:line`）；**永远不发布总结评论**。`【建议】`/`【确认】`/`【提示】`/已核对无问题项只在对话结论里给用户看，不发到 PR。若没有 `【必改】`，不调用评论接口，并告知用户”无必改，不发布 PR 评论”。

**默认禁止**：不要运行测试、构建、启动服务、修改业务代码或格式化文件；除非用户明确要求验证、修复或执行测试。

### 多代理维度划分

评审代理方向参考官方 pr-review-toolkit 插件（anthropics/claude-code 仓库 `plugins/pr-review-toolkit`）的 6 个专家 agent，按本项目（Java 后端、只读评审）适配。每个评审代理只审查一个维度，避免单视角漏检。按 PR 规模裁剪：小 PR 至少派 **通用评审、静默失败** 两个；大 PR（改动文件多 / 跨模块 / 涉数据）派全部；PR 明显不涉及的维度不派发（如纯注释改动不派测试覆盖、无新增类/类型改动不派类型设计）。

| 维度（参考 agent） | 该代理只关注 | 典型检查点 |
|---|---|---|
| 通用评审（code-reviewer） | 项目规范合规与明显 bug | CLAUDE.md / 仓库约定（命名、分层、异常、日志、测试惯例）、风格违规、逻辑错误、空值处理、竞态、安全漏洞、性能问题 |
| 静默失败（silent-failure-hunter） | 错误处理与静默失败 | 空 catch、catch 后仅记日志继续、出错返回默认值/兜底且用户无感知、不当回退、错误消息是否可操作、错误是否该上抛、日志上下文是否够排查 |
| 测试覆盖（pr-test-analyzer） | 测试覆盖质量与缺口 | 行为覆盖 vs 行覆盖、关键路径/边界/异常分支未测、负向用例缺失、并发/异步行为未测、测试测实现而非行为、脆弱测试 |
| 类型设计（type-design-analyzer） | 类/类型设计与不变量 | 封装（内部可变细节暴露）、不变式表达与执行（构造校验、变更点守卫）、贫血模型、职责过多、非法状态可表达 |
| 注释准确性（comment-analyzer） | 注释准确性与可维护性 | 注释与代码不符、过时/误导注释、已解决未删的 TODO、只复述代码的废话注释、缺关键前提/副作用说明 |
| 简化建议（code-simplifier） | 复杂度与简化（**只报不改**） | 过度嵌套、冗余抽象、重复代码、过度紧凑/聪明代码、不必要注释；给出简化建议，不直接修改代码 |

### 各维度评审细则

六个维度的完整评审细则在 `references/agents/` 下（参考 pr-review-toolkit 的组织方式，每个维度一个参考文档，按需加载）：

```
references/agents/
  code-reviewer.md          # 通用评审
  silent-failure-hunter.md  # 静默失败
  pr-test-analyzer.md       # 测试覆盖
  type-design-analyzer.md   # 类型设计
  comment-analyzer.md       # 注释准确性
  code-simplifier.md        # 简化建议（只报不改）
```

派发对应维度子代理时，先读对应文件，把其内容并入该子代理的提示词（子代理用通用 `subagent_type`，不带自定义 agent 类型——这些是参考文档，不是注册的 subagent）。细则与维度表冲突时以参考文档为准，参考文档与下文「证明要求」冲突时以「证明要求」为准。

### 证明要求（每个 finding 必须自证成立）

每个代理对其返回的每条 finding 必须写证明，不是”我觉得这里有问题”：

- **触发场景**：具体输入或状态 → 代码实际产生的错误行为（可复现的最小场景）。
- **仓库级证据链**：`file:line` 引用 + 关键调用路径（数据从哪来、流向哪、在哪出错），且必须验证改动与仓库的交互——被调方实现、调用方预期、相关配置、历史约定，diff 内看不出就算证明不完整；必要时 `git show` / `git grep` / `git log` / 读文件核实，不凭 diff 片段猜测。
- **预期对比**：代码现在做什么 vs 应该做什么。
- **意图谦抑**：看似错误但可能是有意的（与需求/仓库约定冲突、行为变更未注明），标 `【确认】` 附行为证据，不强行 `【必改】`——评审不得越权断言作者意图。
- **写不出证明的发现不算发现**：无证明的疑点最多标 `【确认】`，不得标 `【必改】`；证明有误的条目直接删除。
- **评审默认严格**：只报告有仓库级证据、能复现触发场景的 `【必改】`；证明确凿度不足以支撑”必改”的，一律按实际级别降级，不因”可能有用”保留。
...
- 评论按严重程度分级、行首用 `【必改】` / `【建议】` / `【确认】` / `【提示】` 标注；每条给 `file:line` + **证明** + 原因 + 具体修复；拿不准标 `【确认】` 不标 `【必改】`；主观偏好标 `【提示】` 且不强求。
- **文字风格（深入浅出）**：评审文字中文撰写，能用中文表达的概念不用英文（类名/方法名/字段名等标识符保留原文）；讲清问题与影响用平实的话，不堆大段代码贴片。

完整行内模板、对话结论模板与分级定义见该参考文件。

**非安全保证**：评审结论是静态审查的产物，不代表代码安全或无缺陷——"无必改"不等于可放心合并，关键路径仍需人工复核。对话结论固定保留此声明。
...
- **标注来源**：`Changes` 每条行内只使用 `【AI】` 或 `【人工】`，不使用任何混合标签；末尾"代码来源声明"汇总整体 AI / 人工比例。
...
- **`【必改】` 没有证明或证明站不住**：写不出触发场景 / 引用的 `file:line` 与代码对不上 / 调用链是猜的 → 重写证明，写不出就降级 `【确认】` 或删除，不得以"我觉得"当必改
